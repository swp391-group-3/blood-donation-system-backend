...
pub fn update() -> UpdateStmt {
    UpdateStmt(crate::client::async_::Stmt::new(
        "UPDATE blood_bags SET component = COALESCE($1, component), is_used = COALESCE($2, is_used), amount = COALESCE($3, amount), expired_time = COALESCE($4, expired_time) WHERE id = $5",
    ))
}
...
impl UpdateStmt {
    pub async fn bind<'c, 'a, 's, C: GenericClient>(
        ...
        let stmt = self.0.prepare(client).await?;
        client
            .execute(stmt, &[component, is_used, amount, expired_time, id])
            .await
    }
}
